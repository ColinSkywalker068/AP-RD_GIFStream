#!/usr/bin/env python3
"""Export an AP pre-codec reference from the exact fresh F03 checkpoint.
...
PATCH8_TREE_SHA256 = "0f94bc919c616f30bdf8b32d1e28f8a80668bae04178aa10a0ee182def40b64f"
PATCH8_SHA256 = "1d9d2c96e773f2e77e5bfdf15e87912402c1de207d0c0ac78a52526b93141bbb"
...
def validate_patch8_contract(
    ...
    """Reject any archive/checkpoint that is not the registered Patch8 state."""
    ...
    if receipt.get("schema") != AP_TRAINING_RECEIPT_SCHEMA:
        raise ValueError("archive lacks the AP training-receipt v2 contract")
    if state.get("schema") != AP_TRAINING_STATE_SCHEMA:
        raise ValueError("checkpoint lacks the AP training-state v2 contract")
    if checkpoint_receipt.get("schema") != AP_TRAINING_RECEIPT_SCHEMA:
        raise ValueError("checkpoint lacks the AP training-receipt v2 contract")
    ...
    for name, expected in expected_receipt.items():
        if receipt.get(name) != expected:
            raise ValueError(f"AP training receipt Patch8 field differs: {name}")
    expected_steps = list(range(29000, 30000, 50))
    if (
    ...
    if (
        ...
        or tree.get("sha256") != PATCH8_TREE_SHA256
    ):
        raise ValueError("AP receipt is not bound to the nine-stage Patch8 tree")
    ...
    require_exact_float(ap_meta.get("q_ap_multiplier"), 0.5, "AP multiplier")
    require_exact_float(ap_meta.get("q_bg_multiplier"), 1.30, "background multiplier")
...
def validate_receipts(
    ...
    if (
        ...
        or len(patch_chain) != 9
        or patch_chain[-1] != PATCH8_SHA256
        or normalized_tree.get("sha256") != PATCH8_TREE_SHA256
    ):
        raise ValueError("producer receipt is not bound to the Patch8 runtime")
...
def export_reference(args: argparse.Namespace) -> dict[str, Any]:
    ...
    ap_training_receipt, ap_codec_meta = validate_patch8_contract(
        checkpoint,
        ap_training_payload,
        codec_meta,
        producer_contract,
...
def main() -> int:
    ...
    summary = {
        "schema": "h007.ap_same_run_reference_export.v1",
        "status": "COMPLETED",
