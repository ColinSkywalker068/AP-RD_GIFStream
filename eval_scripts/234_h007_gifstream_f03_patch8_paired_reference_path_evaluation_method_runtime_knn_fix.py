...
KNN reconstruction is method-specific: the official runtime exactly uses the
Patch5 float64 sklearn KDTree path, while AP uses the Patch8 counted-KNN path
with its already-bound canonical identities passed explicitly.  Decode and
bundle-loading semantics remain shared because they are byte-logically
identical across those runtimes.
...
OFFICIAL_KNN_POLICY = "patch5-float64-sklearn-kdtree"
AP_KNN_POLICY = "patch8-counted-knn-explicit-bound-canonical-ids"
...
torch: Any = None
KDTree: Any = None
ap_counted_knn_indices: Any = None
...
def _load_runtime_dependencies() -> None:
    global np, torch, KDTree, ap_counted_knn_indices, decode_anchor_paths
    global canonical_ids, load_bundle
    if torch is not None:
        return
    import numpy as numpy_module
    import torch as torch_module
    from sklearn.neighbors import KDTree as sklearn_kdtree_class
    from gsplat.compression.h007_clean_runtime import (
        counted_knn_indices as ap_counted_knn_indices_function,
    )
    from gsplat.compression.h007_clean_runtime import (
    ...
    np = numpy_module
    torch = torch_module
    KDTree = sklearn_kdtree_class
    ap_counted_knn_indices = ap_counted_knn_indices_function
...
def _knn_policy(method: str) -> str:
    if method == "official":
        return OFFICIAL_KNN_POLICY
    if method == "ap-gifstream-full":
        return AP_KNN_POLICY
    raise ValueError(f"unsupported path-runtime method: {method}")


def _official_patch5_counted_knn_indices(
    splats: Mapping[str, Any], config: Mapping[str, Any]
) -> Any:
    """Reproduce Patch5 h007_clean_runtime counted KNN exactly."""
    ...
    return torch.from_numpy(
        indices[:, 1:].astype(np.int64, copy=False)
    ).to(anchors.device)


def _method_counted_knn_indices(
    method: str,
    splats: Mapping[str, Any],
    config: Mapping[str, Any],
    bound_canonical_ids: Any,
) -> Any:
    policy = _knn_policy(method)
    if policy == OFFICIAL_KNN_POLICY:
        return _official_patch5_counted_knn_indices(splats, config)
    if policy == AP_KNN_POLICY:
        return ap_counted_knn_indices(
            splats,
            config,
            canonical_ids=bound_canonical_ids,
        )
    raise AssertionError("unreachable KNN policy")
...
def _audit_gop(
    ...
    with torch.no_grad():
        ...
        reference_paths = decode_anchor_paths(
            ...
            reference_decoders,
            reference_config,
            knn_indices=_method_counted_knn_indices(
                method,
                reference_splats,
                reference_config,
                reference_ids,
...
def _self_test() -> None:
    assert _knn_policy("official") == OFFICIAL_KNN_POLICY
    assert _knn_policy("ap-gifstream-full") == AP_KNN_POLICY
    try:
        _knn_policy("unsupported")
    except ValueError:
        pass
    else:
        raise AssertionError("unknown methods must not receive a KNN policy")
        ...
        json.dumps(
            {
                ...
                "status": "PASS",
                "checks": [
                    "official-and-ap-knn-runtime-routing-is-method-locked",
                    "unknown-method-knn-routing-is-rejected",
