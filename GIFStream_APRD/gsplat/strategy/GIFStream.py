...
class GIFStreamStrategy(Strategy):
    ...
    def _grow_gs(
        ...
        for i in range(self.update_depth):
            ...
            # The deterministic path contract (and canonical voxel IDs) forbid
            # exact-duplicate anchor rows.  The coarse-grid filter above cannot
            # catch a candidate that lands float-exactly on an existing anchor
            # from a different hierarchy level, so drop those collisions here.
            if candidate_anchor.shape[0] > 0:
                exact_dup_chunks = []
                for j in range(0, params["anchors"].shape[0], 4096):
                    exact_dup_chunks.append(
                        (candidate_anchor.unsqueeze(1) == params["anchors"][j:j+4096].unsqueeze(0)).all(-1).any(-1)
                    )
                keep_exact = ~reduce(torch.logical_or, exact_dup_chunks)
            else:
                keep_exact = torch.zeros(0, dtype=torch.bool, device=candidate_anchor.device)
            candidate_anchor = candidate_anchor[keep_exact]
            ...
            if candidate_anchor.shape[0] > 0:
                ...
                new_feat = scatter_max(new_feat, inverse_indices.unsqueeze(1).expand(-1, new_feat.size(1)), dim=0)[0][remove_duplicates][keep_exact]
                new_time_features = scatter_max(new_time_features, inverse_indices.unsqueeze(1).expand(-1, new_time_features.size(1)), dim=0)[0][remove_duplicates][keep_exact]
                new_factors = scatter_max(new_factors, inverse_indices.unsqueeze(1).expand(-1, new_factors.size(1)), dim=0)[0][remove_duplicates][keep_exact]
