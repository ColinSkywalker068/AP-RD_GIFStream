...
class EntropyCoder():
    ...
    def set_stream(self, stream):
        self._h007_counted_stream = bytes(stream)
    ...
    def decode_stream(self, indexes, cdf, cdf_length, offset):
        ...
        decoded = np.asarray(rv, dtype=np.int32).reshape(-1)
        if not hasattr(self, "_h007_counted_stream"):
            raise ValueError("entropy decoder lacks its counted source stream")
        self.encoder.reset()
        self.encoder.encode_with_indexes(
            decoded,
            np.asarray(indexes, dtype=np.int32).reshape(-1),
            cdf,
            cdf_length,
            offset,
        )
        rebuilt = bytes(self.encoder.flush())
        self.encoder.reset()
        if rebuilt != self._h007_counted_stream:
            raise ValueError("entropy stream is not the exact canonical re-encoding")
        return torch.Tensor(decoded).reshape(1, -1, 1, 1)
